...
public:
	...
	void Render(){
		...
		const auto& list = am.GetAudioList();
		...
		for (auto& it : list){
			auto dist = it.second->GetDistPerRef();
			//22500 = 150* 150
			...
			Vec3 pos =  it.second->GetPosition();
			Color color;
			...
			char buf[255];
			sprintf_s(buf, "File: %s", it.second->GetAudioBuffer()->mFilepath.c_str());
			renderer.QueueDrawText(Vec2I(x + 40, y), AnsiToWide(buf), color, fontSize);
			y += 24;
			sprintf_s(buf, "Gain: %.2f", it.second->GetGain());
			renderer.QueueDrawText(Vec2I(x + 40, y), AnsiToWide(buf), color, fontSize);
			...
			else if (it.second->GetStatus() == AudioSourceStatus::Waiting){
				++numWaiting;
			}
		}
