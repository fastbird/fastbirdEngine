...
#include <Engine/ColladaImporter.h>
	...
	bool mergeMatGroups, bool keepMeshData, bool generateTangent, bool meshGroup)
{
	...
	if (strcmp(filepath, mFilepath.c_str()))
	{
		...
		if (!successful)
		{
			Error(FB_DEFAULT_DEBUG_ARG, FormatString("Importing dae(%s) is failed!", filepath));
		}
	}
	else
	{
	...
	else
	{
		if (mMeshGroup)
		{
			for (auto& animData : mAnimData)
			{
				std::string actionFile = StripExtension(filepath);
				actionFile += ".actions";
				auto toLocal = mMeshGroup->GetToLocalTransform(animData.first.c_str());
				animData.second.ToLocal(toLocal);
	...
	{
		mMeshGroup->SetAuxiliaries(-1, mAuxil);
		mMeshGroup->SetCollisionShapes(mCollisions);
	}
